...
void RigidMeshDeformer2D::SetDeformedHandle(unsigned int vertID, const Wml::Vector2f & vHandle) {
	// Creates a new handle with vertex ID and the x-y coordinates of the vertex
	Constraint c(vertID, vHandle);
	UpdateConstraint(c);  
}

void RigidMeshDeformer2D::removeHandle(unsigned int vertID) {
	Wml::Vector2f zeroVector(0, 0);
	Constraint c(vertID, zeroVector);
	// Removes constraint (handle) and resets the vertex position to its original before deformation
	m_vConstraints.erase(c);
	m_vDeformedVerts[vertID].vPosition = m_vInitialVerts[vertID].vPosition;
	InvalidateSetup();
}
...
void RigidMeshDeformer2D::initializeFromMesh(TTextureMesh* m_mesh) {
	...
	for (int i = 0; i < nTris; ++i) {
		...
		int edge1 = m_mesh->face(i).edge(0);
		int edge2 = m_mesh->face(i).edge(1);
...
void RigidMeshDeformer2D::UpdateDeformedMesh(TTextureMesh* m_mesh, bool bRigid) {
	...
	for (unsigned int i = 0; i < nVerts; ++i) {
		...
		m_mesh->vertex(i).P().x = vNewPos.X();
		m_mesh->vertex(i).P().y = vNewPos.Y();
	}
...
void RigidMeshDeformer2D::UpdateScaledTriangle(unsigned int nTriangle) {
	...
	// Deformed v0 and v1 from solution vector
