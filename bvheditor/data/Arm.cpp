...
int Arm::get_size(){
	...
	std::vector<Segment*>::iterator iter, end;
	for (iter = segments.begin(), end = segments.end(); iter != end; ++iter) {
		count++;
	}
	return count;
}
...
vector<Segment*> Arm::get_segments() {
	return segments;
}
...
//draw according to an externally appointed base point. ignore the arm's own base point
void Arm::draw(float x, float y, float z) {

	Point3f external_base = Vector3f(x,y,z);
	// wireframe mode
	//glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
	// to go back to normal mode
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	...
	*/
	...
	int seg_size = segments.size();
	Point3f start_point = external_base;
